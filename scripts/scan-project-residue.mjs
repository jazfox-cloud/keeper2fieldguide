...
const neutralAllowedHosts = new Set(['example.invalid', 'store.example.invalid', 'www.googletagmanager.com', 'www.sitemaps.org', 'www.w3.org', 'schema.org', 'www.youtube-nocookie.com']);
...
export function scanText(text, allowedIdentities = [], approvedHosts = []) {
  let candidate = text;
  const allowedHosts = new Set([...neutralAllowedHosts, ...approvedHosts]);
...
export async function scanPaths(root, allowedIdentities = [], approvedHosts = []) {
  ...
  for (const relative of targets) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        for (const message of scanText(text, allowedIdentities, approvedHosts)) findings.push({ file: path.relative(root, file), message });
      }
    } catch (error) {
